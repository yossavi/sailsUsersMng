module.exports = function (localFncs) {
...
// Load authentication protocols
	passport.protocols = require('./protocols')(localFncs);
	...
	passport.connect = function (req, query, profile, next) {
		...
		Passport.findOne({
			provider: provider, identifier: query.identifier.toString()
					...
					var findEmail = function () {
						User.findOne({
							...
							if (err) {
								...
								return next(new Error('This email already exists. So try logging in.'));
							} else {
								createUser();
							}
						});
	...
	passport.endpoint = function (req, res) {
		...
		if (!strategies.hasOwnProperty(provider)) {
			req.flash('error', 'Error.Passport.Provider.NotFound');
		}
